...
const app = express();
// const port = 25601; // Nicht mehr benötigt, Vercel bestimmt den Port
// const { bin } = require("cloudflared"); // Nicht mehr benötigt
// const { spawn } = require("child_process"); // Nicht mehr benötigt

// spawn(bin, ["tunnel", "run", "Potentastic"], { stdio: ['pipe', 'pipe', process.stdout] }); // Nicht mehr benötigt
...
async function initializeLeaderboardFiles() {
    ...
    for (const [mode, file] of Object.entries(LEADERBOARD_FILES)) {
        try {
            ...
            //const dir = path.dirname(file); // /tmp sollte immer existieren
            //await fs.mkdir(dir, { recursive: true }); // Nicht nötig in /tmp
...
// Starte den Server nur, wenn er nicht in der Vercel-Umgebung läuft
// Vercel startet den Server selbst
